...
load_dotenv() # This loads local .env files
api_key = os.environ.get("OPENAI_API_KEY") # This works for both local and Render
app = Flask(__name__)
...
def get_vocab():
    ...
    # Change the prompt specifically for Vocab!
    prompt = "Extract the key vocabulary terms from this image and provide definitions for each."

    response = client.responses.create(
        model="gpt-4o-mini", # Note: Corrected the model name below
        input=[
            {
                "role": "user",
                "content": [
